...
void lerArv(lista &Real, string tempStr, ifstream &arquivo){
    char auxc;
    arquivo >> noskipws >> auxc;
    if(arquivo.eof()) return;
    else if(auxc!=0){
        Real.inserirCodigo(auxc, tempStr);
    }
    else{
        tempStr+='0';
        lerArv(Real, tempStr, arquivo);
        tempStr.pop_back();
        tempStr+='1';
        lerArv(Real, tempStr, arquivo);
        tempStr.pop_back();
...
int main(){
    ...
    // L  arvhuf.txt E CRIA UMA LISTA COM OS CARACTERES E SEUS C”DIGOS

    arq.open("arvhuf.txt", ios::in);
    if(!arq.is_open()){
    ...
    while(!arq.eof()){
        lerArv(codigos, auxStr, arq);
    }
    codigos.printCodigos();

    arq.close();

    // CRIA saida.txt E ESCREVE

    arq.open("texto.txt", ios::in);
    if(!arq.is_open()){
        cout << "Erro na abertura1" << endl;
        return -1;
    }

    arqSaida.open("saida.txt", ios::out);
    if(!arqSaida.is_open()){
        cout << "Erro na abertura2" << endl;
        return -1;
    }
    ...
    while(arq.good() && !arq.eof()){
        arq >> noskipws >> c;
        auxStr+=c;
        if(codigos.buscarCodigo(auxStr)!=NULL){
            arqSaida << codigos.buscarCodigo(auxStr)->getChave();
            auxStr.clear();
        }
    }
